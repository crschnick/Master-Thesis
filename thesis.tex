...
\begin{description}
\item[Parameters] {~ \begin{itemize}[\indent{}]
\item \texttt{\textbf{lookAheadIterations}}: The amount of iterations to perform when looking forward to obtain a transformation quality value (see \cref{sec:lvsg}). Default value is $1$.
...
Some sparse grid parameters, like the level, also have to be adapted to handle arbitrary surrogate dimensionalities, as a the grid point amount should be decoupled from the used dimension $r$. 
This means that instead of passing a fixed level $\ell$ as a parameter, we instead use \texttt{approxGridPoints} to specify how much grid points the surrogate should approximately have and determine the most appropriate level from there, as it varies greatly for different values of $r$.
...
, and is reponsible for a big chunk of the average gradient change, while the other two active directions are less important and also not axis aligned.
These two less active directions are however not neglible, as their combined eigen value share still is $18.3\%$.


\subsection{Transformation pipeline}

To start off, we first investigate the general quality of reducing transformations with active subspaces and $1 \leq r \leq 6$ to get an idea of the model functions properties with regards to its approximate compounded intrinsic dimensionality.
We limit $r$ to a maximum of $6$, as the transformed sparse grid surrogates are already affected by the curse of dimensionality for $6 < r \leq 10$.
Furthermore, we use finite differences for now, draw $n=10^5$ samples for the active subspace calculation, and use the relative convergence criterion with $\eta=0$, \ie we will perform iterations until the approximation error does not improve anymore.
...
On the left, we can see surrogate sum approximations using a coarse level and on the right we see the fine level equivalents.
...
The range between coarse and fine level surrogate sums for the same reduced dimensionality $r$ gives insight into what part of the approximation error is based on the compounded intrinsic dimensionality property and what part is based on the surrogate qualities/fidelities itself.
For example, we see that for $r=1$ and $r=2$ the range between low-level and high-level surrogates is relatively small.
This means that the main limiting factor here is that the model function can not be approximated well with $r=1$ and $r=2$.
For $5 \leq r \leq 6$, we see that the potential to improve the approximation with higher levels is used in a better way.
Moreover, we see that in most cases, the most improvements can be achieved within the first three or less iterations, as all graphs basically converge after the at most third iteration.
