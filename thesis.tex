...
We observe that we are not able to really improve the approximation by using transformed sparse grids instead of untransformed sparse grids.

As the data has been obtained through simulation, some input parameters were changed using fixed size increments.
Due to the grid aligned nature of some of the components of the available observations, the nearest neighbour method has been modified such that it can be applied on this model.
In some cases when the observation inputs are partly aligned along some form of grid structure, always choosing the nearest neighbours of an observation can cause the method to only select neighbours along one grid axis, especially when the intervals of the grid differ for each dimension.
