...
\item \texttt{\textbf{exitCondition}}: A predicate indicating whether the pipeline iteration loop should exit. This can make use of a variety of conditions, such as a maximum iteration count or a maximum approximation error.
...
\end{mdframed}
%
If we want to steer the transformation generation process into a certain direction, like preferring lower-dimensional transformations, we can introduce a few modifications, such as a penalty term that grows with the reduced dimensionality $r$ of the transformation.
