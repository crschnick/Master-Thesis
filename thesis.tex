...
\captionof{table}{Computed eigenvectors, eigenvalues, and eigenvalue shares of the active subspace matrix for the Ishigami function using the Monte-Carlo method and $10^5$ samples with finite differences.}
...
Even though the eigenvalue share of the first two eigenvectors is around $90\%$, the loss of the approximation quality when cutting of the most inactive eigenvector in the $r=2$ case is very large.
...
\end{equation}
which models the weight of a wing for a light aircraft based on ten inputs that are defined in \cref{tab:ww_params} and  has also been already studied in the context of sensitivity analysis in \cite{Moon2012}.
\begin{mdframed}[style=style]
\begin{figure}[H]
\vspace{-2mm}
\centering
...
\end{mdframed}
%
To get an idea of the general structure and importance of the inputs, we apply the active subspaces method on this function as well, which yields the results that are shown in \cref{tab:ww_as}.
\begin{mdframed}[style=style]
\begin{figure}[H]
\vspace{-2mm}
\centering
...
To start off, we first investigate the general quality of active direction transformations for $1 \leq r \leq 10$ to get an idea of the model function properties with regards to its approximate compounded intrinsic dimensionality.
To achieve this, we use active subspaces information from \cref{tab:ww_as} and use the relative convergence criterion with $\eta=0$, \ie we will perform iterations until the approximation error does not improve anymore.
...
Moreover, we see that in most cases, the majority of approximation improvements can be achieved within the first three pipeline iterations, as most error graphs converge after the third iteration.
...
We distinguish between dynamic pipeline results that use a different amount of grid points per iteration (gp/it).
...
This time however, the results should be way worse as finding good active directions in ten dimensions is more difficult.
...
Therefore, we can already assess whether random active directions are viable by only performing one pipeline iteration.
...
As the random approach is subject to the curse of dimensionality, it is not feasible for higher-dimensional models.
...
\begin{figure}[H]
\vspace{-2mm}
\includegraphics[width=\textwidth]{graphics/ww_compare}\vspace{-4mm}
\delimit
\captionof{figure}{Approximation errors of various transformed surrogate sum approximations for the wing weight function created using a dynamic pipeline.
The best results from the static pipeline are also included for reference.}
\label{fig:ww_comp}
