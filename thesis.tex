...
If the gradient can not be calculated analytically and can not be approximated using finite differences because function evaluations are very costly, one solution would be to roughly approximate the gradient by looking at neighboring sample points.
Note that while these approximated gradients are of course not as accurate, the resulting Active Subspace results are still useful as shown later in this section.

Let $m \in \mathds{N}$, $x \in \Omega$ and $x_i \in \Omega \setminus \{x\}, ~ i \in \{1, \dots, m\}$. Then
\begin{equation}
\begin{bmatrix}
    x_1 - x\\
    \vdots \\
  ...
  \end{bmatrix}  \nabla_x f =\begin{bmatrix}
    f(x_1) - f(x) \\ \vdots \\  f(x_m) - f(x)
    \\
  \end{bmatrix}
\end{equation}
where the $x_i \in \Omega, x_i \neq x, i \in \{1, \dots, m\}$ are other sample points.
The dimension of this sle is ...

One limitation of this calculation is the linear nature of the calculated gradients, i.e. estimating gradients of a non-linear function leads to biased gradients.
Furthermore, the choice of the $x_i$ heavily influences the result as well as the size of $m$, since the solving complexity is $\mathcal{O}(n\log{}n)$ 
...
For every element $x \in S$, it then randomly chooses $m < n$ elements out of $S \setminus \{x\}$, which are called a random neighbour set $\{x_1, \dots, x_m\}$ of $x$.
This random neighbour set is then used as an input for \ref{} to approximate the gradient $\nabla_x f$.

Overall, one might think that this method will lead to considerably worse results.
However, at the end only the average of the outer products of the gradients is needed for the Active Subspace construction.
Therefore, general trends of the gradients should still be reflected with this method.

For $n \to \infty$, the approximated gradient does not converge to the actual gradients.

\subsection{Nearest neighbour approximation}

The nearest neighbour variant picks a subset $S' \subseteq S$ with a manageable size $n'=|S'|$ and calculates the neighbours $x_1,\dots,x_m$ of a point $x$ by selected all points with a norm value smaller than ... .
The random neighbour variant ...
Overall, one might think that these several limitations of these methods lead to worse results.
However, at the end only the average of the outer products of the gradients is needed for the Active Subspace construction.
...
Compared to the random neighbour approximation, for $n \to \infty$, the approximated gradient does converge to the actual gradient.
...
\begin{center}
	\includegraphics[width=\textwidth]{graphics/ReductionPipeline.pdf}
\end{center}
	\captionof{figure}{Die eindimensionale hierarchische Prewavelet-Basis eines Dünnen ANOVA-Gitters für die Level -1 bis 3}
	\label{fig:ansatz_prewavelet}
\end{figure}
...
\section {Transformation generator}
...
\subsection {Stream transformation generator}

Since for every transformation generation method there are always many possible transformations to choose from, as will be shown later on, the transformation generation process is implemented using streams.
A transformation stream generator will sequentially generate as much different transformations as it can.
Every transformation is evaluated using a transformation evaluator and the best transformation will be chosen and returned in the end.
...
Since the probability of finding a good transformation randomly in one try is very low, it makes sense to also introduce an iterative transformation generator, which can be combined with any other transformation generator.
...
The already described Monte-Carlo Active Subspace method

\begin{figure}[H]
	\includegraphics[width=\textwidth]{graphics/TransformationStreamGen_AS.pdf}
\captionof{figure}{Component structure of an Active Subpsace transformation stream generator}
\end{figure}
...
\begin{description}
\item[Sample count:] The amount of samples used to create a gradient sample and subsequentially the Active Subspaces.
\item[Gradient sample generator:] The maximum amount of used eigen vectors.  The maximum amount of used eigen vectors.  The maximum amount of used eigen vectors.
\item[Construction parameters:] {~ \begin{enumerate}[\indent{}]
\item \textbf{Min dimensions} The minimum amount of used eigen vectors.
\item \textbf{Max dimensions} The maximum amount of used eigen vectors.
\item \textbf{Min eigen value share (+)} Scientific study of mental processes and behaviour.
\end{enumerate}}
\end{description}

\end{mdframed}
\captionof{figure}{Active Subpsace transformation stream generator configuration options}
\end{figure}


In the Active Subspace transformation creator step, the transformation evaluator is used to test many different possible transformations that can be created by using the eigenvectors and eigenvalues.
...
It allows us to ...
It can also serve as a reference for comparision as one would expect that the Active Subspace method would always output better transformation.
...
\begin{definition}[Random config]
...
So for a random transformation generator, two variants can be chosen:
\begin{enumerate}
\item Random basis, best transformation using evaluator $e$.
\item Random basis, random reduced dimensions
\end{enumerate}

\subsection {Active Subspace transformation creator}

Usually, the eigenvectors are sorted by the magnitude of the eigenvalues and then only 
