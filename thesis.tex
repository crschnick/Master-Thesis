...
To actually compute the active directions, a set of observations $S \coloneqq \{x_i, f(x_i)\}_{i=1}^n$ is randomly drawn from $\rho$ and used to derive the matrix $C$ with the help of a classical Monte-Carlo approach from \cite{Constantine2014}.
...
\vspace{-1.5mm}
\delimit
	\captionof{algorithm}{Pseudocode of the iterative transformed surrogate sum algorithm. Parameters are the model function $f$.}
	\label{alg:itappr}
\end{mdframed}
%
...
\vspace{-1.5mm}
\delimit
	\captionof{algorithm}{Pseudocode of the transformation generation algorithm. Parameters are the current error function $e$, the minimum reduced dimensionality $r_{\text{min}}$, the maximum reduced dimensionality $r_{\text{max}}$, and the penalty exponent $\gamma \geq 0$.}
	\label{alg:besttrans}
\end{mdframed}
%
