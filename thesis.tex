...
\Cref{chap:c5} then combines the input transformations of the previous chapters with an iterative approach to obtain the more powerful compounded transformed surrogates.
...
\Cref{chap:c9} concludes this thesis by providing a summary of the newly introduced technique and its results.
...
Functions that fullfil this property are called radial functions and the associated univariate function $\varphi$ is called a radial kernel centered at $c$.
...
For this purpose, even the relatively simple process of drawing samples from the input parameter probality distribution of the model can improved to achieve better results.
...
\end{definition}
%
The RMSE is closely related to the definition of the continous $L^2$ error, as it is just a discrete version of it in case the observations $S$ are uniformly distributed.
Thus, it holds that
\begin{equation}
...
To improve upon these absolute error metrics and allow for comparisions across model functions with different ranges of function values, we define normalized error metrics.

\begin{definition}[Normalized root mean squared error]
Let $\hat{f}$ be a model function surroate for $f$ and $S=\{(x_i, f(x_i)\}_{i=1}^n \subseteq \Omega \times \mathds{R}$ a set of observations.
We define
\begin{equation}
...
\end{equation}
\label{def:intrinsic}
Even though we do not explicitly define $\Omega_r$, it should also be a bounded Euclidian product such that it can  transformed into the unit hypercube $\Omega_r=[0,1]^r$, similar to $\Omega$.
\end{definition}
%
...
\end{subfigure}
  \delimit
\caption{
Approximation error comparision between spatially adaptive sparse grids and radial basis functions for both example functions $f_1$ and $f_2$.}
...
\end{tabular}
\delimit
  \captionof{table}{Sobol indices $S_c$ for all ANOVA components of $f_1$.}
  \label{tab:anova_f1}
    \end{minipage}%
    \hspace{.05\textwidth}
...
\end{tabular}
\delimit
  \captionof{table}{Sobol indices $S_c$ for all ANOVA components of $f_2$.}
  \label{tab:anova_f2}
    \end{minipage}
\end{figure}
\end{mdframed}
...
\begin{minipage}{.4\textwidth}
  ...
  The outline of the zonotope is coloured in \darkbluecomma and the origin in \reddot}
  \label{fig:zonotope}
    \end{minipage}%
...
The generators $q_1, \dots, q_r$ of the encasing hypercube can be calculated by applying the Gram–Schmidt process on the ordered zonotope genrators $p_i$ to obtain the orthogonal vectors,
\begin{equation}
...
\begin{minipage}{.49\textwidth}
  ...
  The outline of the projected zonotope $Z_p$ is coloured in \lightbluecomma the encasing surrogate space $S$ in dotted \darkbluecomma and the origin in \red.}
...
The random neighbour approximation method takes a subsample $S_{\text{rn}} \subseteq S \setminus \{(x, f(x))\}$ with $|S_{\text{rn}}|=m < n$ as input.
...
It can also serve as a reference for comparision, as the active subspace method should almost always output better active directions.
...
If the model function is for example periodic along one dimension $i$ with the period $p_i$, \ie it holds that $f(x_1,\dots,x_i,\dots,x_d) \approx f(x_1,\dots,x_i + p_i, \dots,x_d)$, we can easiliy define a transformation function $t_{\text{periodic}} \colon \Omega \mapsto \Omega$ with
\begin{equation}
...
Manifold-based methods for dimensionality reduction aim to identify nonlinear subspaces in the domain that are suitable for a dimensionaliy reduction, similar to active subspaces.
...
\begin{minipage}{.49\textwidth}
        \centering
  \captionof{figure}{Examplary case of an active direction transformation with $r=d=2$. The \beige square represents $\Omega=\Omega_r$, the black dots are the grid points of a sparse grid surrogate, and the rotated \lightblue square is $T$. The \grey square in top right corner represents the basis function support of the grid point in its center.}
...
\label{eq:ppr}
\end{equation}
where $\beta_0$ is a constant, $\beta_k$ are projection matrices, the $f_k$ are lower dimensional functions, and the $\epsilon_i$ are the residiuals.
...
\begin{algorithmic}[1]
    ...
    \For{$i = 1, \dots,  \infty$}
    	...
    	\State $\hat{f}_{t_i} \gets$ \texttt{generateBestSurroate}($e_{i - 1}, t_i, S_{t_i}$)
    	\State $e_i \gets e_{i - 1} - \hat{f}_{t_i}$
    	\State $S_i \gets \{\left(x_j, e_i(x)\right)\}_{j=1}^n$
    	\If{\texttt{shouldExit}$(e_i)$}
...
\vspace{-1.5mm}
\delimit
	\captionof{algorithm}{Pseudocode of the iterative transformed surrogate sum algorithm. Parameters are the model function $f$ and the amount of used obervations $n$.}
	\label{alg:itappr}
\end{mdframed}
%
...
\Cref{alg:bestsur} shows a simple implementation that uses low-fidelity surrogates for parameter evaluation to reduce runtime to determine the optimal configuration,
\begin{equation}
...
This means that we can always come reasonably close to any specified grid point amount through choosing the right level and performing the right amount of refinements such that we can accurately compare approximation errors of sparse grid surrogates across all possible reduced dimensionalties.
...
\vspace{-1.5mm}
\delimit
	\captionof{algorithm}{Pseudocode of the transformation generation algorithm. Parameters are the current error function $e$, the minimum reduced dimensionality $r_{\text{min}}$, and the maximum reduced dimensionality $r_{\text{max}}$.}
	\label{alg:besttrans}
\end{mdframed}
%
...
In theory, an optimal method should be able to approximate $f$ without a resultung in a leftover error function after two iterations, as it consists out of two terms and its compounded intrinsic dimension is $1$.
...
\end{description}
\delimit
\caption{Component structure of an random transformation stream generator and the associated configuration paremeters.}
\label{fig:rtsg}
\end{figure}
\end{mdframed}
%
...
This guarantees that sparse grids of different dimensionalities all contain an almost equal amount of grid points, which is very useful when performing comparisions.
...
RBF surrogates can be created with the interpolation approach, as they can interpolate from arbitrary function obervations $S_t$.

\section {Evaluators}

To judge the quality of transformations and surrogates, we introduce evaluator components.
These evaluator components take a transformation or surrogate as an input and output a real number that represents their quality and allows for comparisions with others.

\subsection {Surrogate evaluator}
\label{sec:se}

A surrogate evaluator is a simple component that computes the surrogate approximation error by using validation observations and an error metric to compute the Monto-Carlo approximation error of the surrogate, \eg the MSE, RMSE, and NRMSE.
...
For example, the generator components and evaluator components should all work on different observations such that no overtting can possibly take place.
...
The observation subset, which is passed down by each component to the next ones is also randomly shuffled, such that partioning them into train and validation sets is not deterministic when performing multiple pipeline iterations.
...
As many components are designed to be forward looking, we can think of the transformation pipeline as a collection of different subpipelines.
The high-fidelity subpipeline is responsible for finding the the best and most expensive possible surrogate that will be retuned after each complete pipeline iteration.
It achieves this by usually evaluating the quality of multiple transformations from one family of possible transformations using a look-ahead approach, \ie executing a low-fidelity subpipeline that evaluates the transformation quality by effectively performing a whole iteration with low-fidelity surrogates.
...
The \red box represents the high-fidelity subpipeline components, while the \green box identifies the low-fidelity subpipeline components.}
\label{fig:tpex}
\end{figure}
\end{mdframed}
...
These components interact with each other by passing data down the transformation pipeline, which can cause compounding inaccuracies and errors if the inputs from previous stages are inaccurate.
...
We start off with the low dimensional Ishigami function,
\begin{equation}
...
\captionof{table}{Computed eigenvectors, eigenvalues, and eigenvalue shares of the active subspace matrix for the Ishigami function using the Monte-Carlo method and $10^6$ samples with finite differences.}
\label{tab:ishigami_as}
\end{figure}
\end{mdframed}
%
Based off the values in \cref{tab:ishigami_as}, we can see that the most active direction almost exactly corresponds to second input dimension and is reponsible for a big chunk of the average gradient change, while the other two active directions are less important and are also not axis aligned.
These two less active directions are however not neglible, as their combined eigenvalue share still is $18.3\%$.

\subsection{Dimensionality preserving transformations}

While the main focus lies on reducing transformations with $r < d$, we still take a short look at the quality of simple dimensionality preserving transformations with $r=d$, which are effectively only changing the orientation of the function input parameters.
For the orientation change, we use all computed active subspace directions from \cref{tab:ishigami_as}, or in other words apply the active direction transformation $t_{W_3}$ on the inputs.
...
In this section, we are therefore interested in answering the following questions:
\begin{enumerate}
...
\end{enumerate}
\Cref{fig:ishigami_errors} shows the first comparision results of untransformed and transformed sparse grid surrogates created using interpolation or regression.

\begin{mdframed}[style=style]
\begin{figure}[H]
\includegraphics[width=\textwidth]{graphics/ishigami_orientation}\vspace{-2mm}
\delimit
...
\end{mdframed}
%
As we can see, the pure orientation change does not lead to better interpolation results.
...
As we do not perform a dimensionality reduction in this example, even if it would be feasible, this results in a constructed surrogate space that covers a lot more than the required transformation space.
...
Furthermore, the introduced regression penalty, compared to the interpolation method, is a factor of approximately $10^1$.
...
Therefore, we will continue to only use the modified hat basis for the Ishigami function for the rest of this evaluation section, as it requires less grid points.
...
As the tansformation process focuses on reducing transformations that have to deal with unused space and can only use regression, this means that the individual transformed sparse grid surrogates do not become worse just through the orientation change, but allow for a much more flexible dimensionality reduction process.
Therefore, dimensionality reducing transformations seem to have a much bigger potential than orientation changing ones.
...
We can therefore not expect to see any gain from applying a transformation with $r < d$, as this will only occur for higher dimensionalities.
Even though reducing transformations should not be competitive, we are still interested in answering some basic questions:
\begin{enumerate}
...
\end{mdframed}
%
As we can see in \cref{fig:ishigami_red}, the dimensionality reducing transformations lead to, as expected, way worse results and do not provide viable alternative surrogates.
...
Even though the eigenvalue share of the first two eigen vectors is around $90\%$, the loss of the approximation quality when cutting of the most inactive eigen vector in the $r=2$ case is very large.

\subsection{Iterative transformations}

Until now, we only performed one iteration when creating transformed surrogates.
...
\newpage
\noindent To judge the quality of approximated gradients, we first have to define an error metric for them:
...
At first glance, using some sort of approximated active subspace deviation looks like a good metric to evaulate approximated active subspaces.
...
Also, if two eigen values $\lambda_i, \lambda_j$ are similar in magnitude, the order of eigenvectors might switch arbitrarily and result in a huge matrix norm value.
...
The random neighbour methods gradient errors all lie in a range that is similar to the finite differences approximation with a very big step size of $h=5$.
...
To verify the quality of the active subspace method compared to the true optimal linear transformation, which we can't necessarily construct deterministicly using active subspaces, another approach are random active direction transformation (see \cref{sec:rt}).
...
For the Ishigami function in the low-fidelity case, we use a sparse grid surrogate generator with modified linear basis functions, and a value of \texttt{approxGridPoints}$=250$.
In the high-fidelity case, we increase the grid point amount to \texttt{approxGridPoints}$=1000$.
...
\begin{subfigure}{.5\textwidth}
  ...
  \captionof{figure}{Interpolated transformed sparse grid surrogates with $r=3$. The resulting high-fidelity error is $\varepsilon^\mathrm{h}_{\mathrm{min}}=0.0007$.}
\vspace{3mm}
...
\begin{subfigure}[H]{.5\textwidth}
  ...
  \captionof{figure}{Regressed transformed sparse grid surrogates with $r=3$. The resulting high-fidelity error is $\varepsilon^\mathrm{h}_{\mathrm{min}}=0.031$.}
\vspace{3mm}
...
\end{subfigure}
\delimit
  \captionof{figure}{Histogram of low-fidelity approximation errors for $10^4$ randomly generated transformations and the associated best possible high-fidelity error.}
\label{fig:ishigami_hists}
\end{figure}
\end{mdframed}
%
\Cref{fig:ishigami_hists} shows the approximation error distribution of interpolation and regression surrogates.
...
\vspace{0.5em}
\delimit
\captionof{table}{Input parameters for the wing weight model.}
\label{tab:ww_params}
\end{figure}
\end{mdframed}
%
...
\captionof{table}{Computed eigen values, and eigen value shares of the active subspaces for the wing weight function using the Monte-Carlo method and $10^5$ samples.}
\label{tab:ww_as}
\end{figure}
\end{mdframed}
%
Based off the values in \cref{tab:ww_as}, we can see that there exists one dominant active direction that takes up the majority of the eigenvalue shares.
...
However, as we learned for the Ishigami function, these eigenvalues should only be used as rough indicators and not as reliable sources about the suitability for dimensionality reduction using transformed sparse grids.

\subsection{Static pipeline}

To start off, we first investigate the general quality of reducing transformations with active subspaces and $1 \leq r \leq 10$ to get an idea of the model function properties with regards to its approximate compounded intrinsic dimensionality.
To achieve this, we use active subspaces with a finite differences gradient generator for now, draw $n=10^5$ samples for the active subspace calculation, and use the relative convergence criterion with $\eta=0$, \ie we will perform iterations until the approximation error does not improve anymore.
...
\delimit
\captionof{figure}{Approximation errors of various transformed surrogate sum approximations for the wing weight function.}
\label{fig:wwf}
\end{figure}
\end{mdframed}
%
...
For $r =5, \dots, 6$, we see that the potential to improve the approximation with higher levels is used in a better way.

Moreover, we see that in most cases, the majority of approximation improvements can be achieved within the first three iterations, as most graphs converge after the third iteration.
In this case that uses static pipeline iterations, we do not have the freedom to optimally choose the best reduced dimensionality in each iteration, which might limit the effectiveness of multiple iterations.

Another interesting trend that can be observed is the behaviour of the approximation errors as $r$ grows.
The general approximation quality improves from $r=1$ to $r=7$ but hits a point at which the trend reverses and then worses from $r=7$ to $r=10$.
This can be explained by the general tradeoff between grid point resolution and dimensionality.
As higher-dimensional grids effectively have to spend less grid points for each dimension when keeping the amount of grid points approximately constant, this results in a tradeoff between grid points spent per dimension and reduced dimensionality.
This optimal tradeoff point is hit at $r=7$ in this case.

All in all, we see that higher reduced dimensionalities in combination with a lower sparse grid level provides the best approximation error tradeoff up until $r=7$.
...
\Cref{fig:ww_hist_all} shows the low-fidelity approximation error distribution for random active direction generators and the high-fidelity approximation error of the best transformation out of all.
...
This behaviour can be explained by the fact that we require even more random generation runs to find a good transformation in higher dimensionalities.
As the approximation errors do not come close the active subspace generation results, the random generation is not a viable alternative.

\subsection{Summary}

To summarise our findings, we can say that the active subspace method performs much better than random active direction generation for this high-dimensional model function.
...
\vspace{0.5em}
\delimit
\captionof{table}{Input parameters for the borehole model.}
