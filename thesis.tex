...
\begin{bmatrix}
  ...
  \label{eq:dd_sle}
\end{equation}
Solving this system of linear equations for a residual minimizing value for $\nabla f(x)$ only provides a very rough approximation of the actual gradient.
...
Therefore, the evaluation of the active subspace approximations will be done later on when the surrogate quality is investigated.
...
\end{subfigure}
\delimit
\caption{Differences in sample selection between the two neighbour approximation methods for examples with $|S|=10$.
