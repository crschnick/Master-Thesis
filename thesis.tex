...
In that case, if a linear transformation is created according to the rules of the previous chapter s.t. all inputs are transformed into a unit hypercube with equal dimension, the transformation function is injective with $t(\Omega) \subseteq \Omega$.
Therefore, it is possible to use normal sparse grid interpolation to create a surrogate for the transformation, if we either define a rule on how to treat function evaluations for grid points outside of $\Omega$ or ensure that grid points outside of $\Omega$ never are created.
...
Another approach would be to create a modified model function $f' \colon \Omega \mapsto \mathds{R}$ that can be evaluated outside of $\Omega$.
...
Note that even if there are many grid points that were generated using spatial adaptivity outside of $T$, as seen in figure \ref{}, no grid point is actually completely unused, i.e. the support of every grid point basis function intersects at least barely with $T$.

\todo{Image}

However, if we perform a dimension reduction with $r < d$, we can no longer effectively make use of sparse grid interpolation as the transformation is longer injective, i.e. many inputs can be projected onto one point with $t(x_1)=\dots=t(x_n)$.
We are then dealing with data that might be, depending on the model to reduce, very noisy and there even might be multiple different values for the same inputs, i.e. $(t(x_i),f(x_i)), (t(x_j),f(x_j)) \in S, ~ t(x_i)=t(x_j), f(x_i) \neq f(x_j)$.
...
As the primary focus of this thesis lies on exploting lower dimensionalities with $r < d$, we focus on the regression method for constructing sparse grid surrogates.
The original input samples $S=\{x_1,\dots,x_n\} \subseteq \Omega$, which were drawn randomly according from the pdf $\rho$, are transformed to obtain the transformed sample $S_t=t(S)$ where the $t_i \in S_t$ transformed inputs with $t_i \in T$.
We are aiming to construct the function $\hat{f}_t$ using sparse grids with the grid point set $X$ and arbitrary basis functions $\phi_{\underline{l},\underline{i}}$ with
\begin{equation}
...
In many cases, a multistep approach that generates a sum of transformed surrogates suits a model function better than a single one.
We can construct a simple example function that shows the limitations of a single step approach.

\begin{equation}
f(x_1, x_2)=\sin(2 \pi x_1) + x_2
\nonumber
\end{equation}

This function is a sum of two one-dimensional terms.
...
\normalsize
\begin{algorithmic}
\Function{generateBestSurroate}{S, C, $\epsilon$}
    \State c*=$\emptyset$
    \State $e_{min}=\infty$
    \For{c : possibleConfigurations}
...
\begin{subfigure}{.45\textwidth}
  ...
  \label{fig:as_nn}
\end{subfigure}
\caption{Second iteration of the iterative transformation algorithm. The eigenvalues computed for this step are $\lambda_1=30.32$ and $\lambda_2=0.98$. Therefore, a one-dimensional reduction makes sense.}
\label{fig:pipeline_2}
\end{figure}
