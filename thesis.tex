...
\chapter{Finding a good basis for transformations}


\section{Estimating gradients}

If the gradient can not be calculated analytically and can not be approximated using finite differences, one solution would be to roughly approximate the gradient by looking at neighboring sample points.
Such an approach is useful for problems such as regression, where we only have a given set of samples.
These gradients can be approximated by solving the following sle:
\begin{equation}
\begin{bmatrix}
    x_1 - x\\
    \vdots \\
...
One limitation of this calculation is of course the linear nature of the calculated gradients, i.e. estimating gradients of a non-linear function leads to biased gradients.
Furthermore, 
...
\todo{Insgesamt noch etwas unübersichtlich. Konsistenter die Bezeichner $\Omega_r$, $S$,$Z,\dots$ verwenden und nicht nur  ``the unit hypercube''}

Furthermore, we also only use a unit hypercube as an input space $\Omega$, since this a commonly used input space and can be compared to normal sparse grids.
To construct a transformation function $t$ for $\Omega=[0,1]^d$ and $\Omega_r=[0,1]^r$, we first define the center  as $c_i=(\frac{1}{2}, \dots, \frac{1}{2})^T\in \mathbb{R}^i$.
Therefore, the inputs are first projected using $p(x)=W_r^T (x-c_d)$ and the result of this projection for all inputs $p(\Omega)$ is an $r$-dimensional zonotope centered at the origion with $d$ generators.
This zonotope corresponds to a $d$-dimensional minkowski sum with
\begin{equation}
Z=\{\sum_{i=1}^d p_i x_i , ~ -\frac{1}{2} \leq x_i \leq \frac{1}{2}\}, ~~ Z=p(\Omega)
\end{equation}
The generators\todo{Der Begriff generator wird oft genutzt aber nirgends definiert} of this zonotope can be obtained by projecting the unit vectors $e_i$ that span the hypercube $\Omega=\{c_d + \sum_{i=1}^d e_i x_i , ~ -\frac{1}{2} \leq x_i \leq \frac{1}{2}\}$ to get
\begin{equation}
p_i=W_r e_i, ~~ |p_1|\geq \dots \geq |p_d|
\end{equation}
However since the surrogate creation process expects inputs from a $r$-dimensional unit hypercube $\Omega_R$, during the normalization step, the zonotope $Z$ is first encased in an $r$-dimensional hypercube called the surrogate space which is then scaled and aligned to obtain the unit hypercube\todo{welchen?}.
...
\nonumber
\end{equation}
\todo{Die Bezeichner $S$ und $Z$ müssen schon weiter oben kommen}The surrogate space $S$ which encases the zonotope $Z$, i.e. $Z \subseteq S$, is then an $r$-dimensional hypercube with
\begin{equation}
S=\{c_r + \sum_{i=1}^r q_i x_i , ~ -s_i \leq x_i \leq s_i\},~~ s_i=\sum_{k=1}^d \langle q_i, p_k\rangle
\label{surrogate_space}
\end{equation}
where the scaling factors $s_i$ make the hypercube $S$ exactly match the extent of the zonotope $Z$ as shown in \ref{fig:space}.
The surrogate space $S$ can then be easiliy transformed into a unit hypercube $\Omega_R$ as shown in \ref{fig:aligned} and therefore \todo{?} elements $z \in Z$ 
\begin{equation}
a(z)=c_r + \Gamma^{-1} Q^T z, ~~
Q=\begin{bmatrix}
  \\
  ...
  \end{bmatrix}
  , ~~
\Gamma=\text{diag}(s_1, \dots, s_r), ~~
\label{alignment}
\end{equation}


The surrogate space $S$ can then be transformed into a unit hypercube $\Omega_R$\todo{$\Omega_r?$} with \eqref{alignment}, i.e. $a(S)=\Omega_R$, and therefore the transformation function can be defined as follows:
\begin{equation}
t_{\text{Cube}}(x)=a(p(x))
\end{equation}
...
\section{Active Subpsace transformation generator}

The already described 

\begin{figure}[H]
\begin{center}
	\includegraphics[width=\textwidth]{graphics/TransformationGen_AS.pdf}
\end{center}
\end{figure}

\subsection {Gradient sample generator}

\begin{figure}[H]
\begin{center}
	\includegraphics[width=\textwidth]{graphics/GradientGen.pdf}
\end{center}
\end{figure}
...
\section {Random transformation generator}
...
\begin{figure}[H]
\begin{center}
	\includegraphics[width=\textwidth]{graphics/TransformationGen_Random.pdf}
\end{center}
\end{figure}
...
\section {Iterative transformation generator}
