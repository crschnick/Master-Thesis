...
\todo{Formalize regression algorithm}
...
\begin{subfigure}{.5\textwidth}
  ...
  \caption{Grid points for $f_1$}
  \label{fig:as_rn}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.8\linewidth]{graphics/grid_f2}
  \caption{Grid points for $f_2$}
  \label{fig:as_nn}
\end{subfigure}

\vspace{2.5mm}
\delimit
\caption{Grid points of generated spatially adaptive sparse grids for $f_1$ and $f_2$ that started out as a regular sparse grid with level $l=1$. It used the surplus refinement criterion, 15 refinements steps with one grid point refinement per step, and modified linear basis functions.}
\label{fig:grids}
\end{figure}
\end{mdframed}
...
Therefore the main approach that will be used and investigated in this thesis starts out with finding an orthonormal matrix $W \in \mathds{R}^{d \times d}$ that is aligned along the active directions of the model function in the best possible way.
We then go one step further and now also try to eliminate the most inactive directions, i.e. performing a dimension reduction, which are also now the most inactive dimensions after aligning the active directions along the axes, we have to possibility of drastically reducing the amount of grid points while only suffering a small increase in error in theory.
...
Let $W \in \mathds{R}^{d \times d}$ be an orthonormal matrix with $W^T W=I$.
Given a reduced dimension $r \leq d$, we define
\begin{equation}
...
\begin{mdframed}[style=style,frametitle={Zonotope}]
\begin{figure}[H]
\centering
...
\end{equation}
results in a projected input space that has the zonotope form of equation \ref{zonotope} where the $p_i$ are the generators with
\begin{equation}
Z_{p}=\{\sum_{i=1}^d p_i x_i , ~ -1 \leq x_i \leq 1\}, ~~ p_i=\frac{1}{2} W_r^T e_i
\end{equation}
and according to equation \ref{zonotope_form}, it holds that the projected input space is exactly the zonotope $Z_p$ with $Z_{p}=p(\Omega)$.
...
\end{equation}
where the scaling factors $s_i$ make the hypercube $S$ exactly match the extent of the zonotope $Z$ as shown in figure \ref{surrogate_space}.

\begin{mdframed}[style=style,frametitle={Surrogate space}]
\begin{figure}[H]
\centering
\vspace{2.5mm}
  \includegraphics[width=0.4\linewidth]{graphics/surrogate_space}
\delimit

  \captionof{figure}{Construction of the encasing surrogate space $S$ for the zonotope $Z_p$ of \cref{fig:zonotope} according to \cref{surrogate_space}. The projected zonotope $Z_p$ is drawn in {\color{light_cornflower_blue} light blue}, the encasing surrogate space $S$ in dotted {\color{charcoal} dark blue}, and the origin in {\color{persian_red} red}.}
...
The surrogate space $S$ can then be easiliy transformed into a unit hypercube $\Omega_R$ as shown in figure \ref{fig:aligned}.
First, we observe that the vectors $q_i$ form an orthogonal basis of $S$ and define
\begin{equation}
Q=\begin{bmatrix}
  \\
...
\end{equation}
as the basis.
We can perform a change of basis on every element $z \in S$ that has been projected into $Z_p$ and therefore also $S$ with $z=p(x), x \in \Omega$ to obtain $v=Q^T z$.
Per surrogate space definition \ref{surrogate_space}, we know that $v_i \in [-1,1]$ holds as $v \in S$.
...
\begin{equation}
t_{W_r}(x)= c_r + \text{diag}\left(\frac{1}{2}, \dots, \frac{1}{2}\right) Q^T p(x)
\label{linear_trans}
\end{equation}
...
\vspace{4.5mm}
\centering
  \includegraphics[width=0.25\linewidth]{graphics/surrogate_space_unit}
\vspace{1.5mm}

\delimit

  \captionof{figure}{Transformation of $S$ into a unit hypercube according to equation \eqref{alignment}.}
  \label{fig:aligned}
\end{figure}
\end{mdframed}
...
The described transformation process can be visualized by illustrating where grid points in the lower dimensional space $\Omega_r$ would reside in the original input space $\Omega$. 
While it is normally not required to revert the previously shown transformation process, i.e. going from $\Omega_r$ to $\Omega$, we require it for the visualization and it is therefore only shown in a more informal way.

Equation \ref{linear_trans} can be rearranged to solve for $p(x)$ with
\begin{equation}
p(x)=(Q^T)^{-1} \text{diag}\left(\frac{1}{2}, \dots, \frac{1}{2}\right)^{-1} (t_{W_r}(x) - c_r)
\label{proj}
\end{equation}
Since the column vectors of $Q$ are orthogonal per equation \ref{q} and $Q$ can be expressed as $Q=\text{diag}\left(s_1, \dots, s_r\right) \mathcal{Q}$ where the column vector of $\mathcal{Q}$ is just the normalized column vector of $Q$ with $\mathcal{Q}_i=\frac{q_i}{\| q_i \|}$ and the $s_i$ are the scaling factors per equation \ref{q_scaled} to obtain
\begin{equation}
\begin{split}
p(x)=&(\text{diag}\left(s_1, \dots, s_r\right)^{-1} \mathcal{Q}) ~ \text{diag}\left(2, \dots, 2\right) (t_{W_r}(x) - c_r)\\
=&\text{diag}\left(\frac{2}{s_1}, \dots, \frac{2}{s_r}\right) \mathcal{Q} (t_{W_r}(x) - c_r)
\end{split}
\end{equation}

To reverse the transformation completely, we have to deal with reversing $p(x)$ and the fact that in many cases the inverse $p^{\text{inv}}(x_p)=\{x \in \Omega \mid p(x)=x_{p}\}$ function is not injective, i.e. $|p^{\text{inv}}(x_p)| > 1$.
If we only consider the set of points that, corrected for the centering, project onto themselves with
\begin{equation}
x_p=p(x) + c_r
\label{proj_rep}
\end{equation}
and reverse the projection $p(x)$ only onto the representative elements $x_p$, we are able to map one element $y \in \Omega_r$ onto one element $x_p \in \Omega_r$.
...
\begin{figure}[H]
\centering
  \includegraphics[width=.5\linewidth]{graphics/aligned_grid}
\caption{Illustration of a possible case with $r=d=2$. The orange square represents $\Omega=\Omega_r$, the black dots are the grid points of a sparse grid surrogate, and the rotated green square is $T$. The orange part not included in the green square is $U$ with $U=\Omega \setminus T$. The brown square represents the support of the grid point in its center.}
...
\begin{figure}[H]
\centering
  \includegraphics[width=.5\linewidth]{graphics/reduced_grid}
\caption{Illustration of a possible case with $r=1$ and $d=2$, where the process in broken down into two steps. The surrounding white square represents $\Omega$, the orange line represents $\Omega_r$, the black dots are the grid points of a sparse grid surrogate, and the rotated green square is $T$.}
...
\normalsize
\begin{algorithmic}
\Function{projectionPursuitRegression}{$f, k_{\text{max}}$}
    \State $e_0 = f$
    \For{$k = 1, \dots, k_{\text{max}}$}
    	\State $\beta_k \gets$ determineBestProjection($e_k$)
    	\State $f_k \gets$ determineBestSurrogate($e_k, \beta_k$)
...
\EndFunction
\end{algorithmic}
	\captionof{algorithm}{Pseudocode of the iterative projection pursuit regression algorithm.}
	\label{alg:ppr}
\end{algorithm}
...
\normalsize
\begin{algorithmic}
\Function{transformedSurrogateSum}{$f, i_{\text{max}}$}
    \State $e_0 = f$
...
\EndFunction
\end{algorithmic}
	\captionof{algorithm}{Pseudocode of the iterative transformed surrogate generation algorithm.}
\end{algorithm}
...
\normalsize
\begin{algorithmic}
\Function{generateBestSurroate}{$S_t, C, \epsilon$}
...
\begin{subfigure}{.45\textwidth}
  ...
  \caption{Error function $e_0$ and first active subspace direction $v_1$ (blue).}
  \label{fig:as_rn}
\end{subfigure}%
...
\begin{subfigure}{.45\textwidth}
  ...
  \caption{Transformed surrogate input samples (blue) and constructed surrogate (red).}
  \label{fig:as_nn}
\end{subfigure}
