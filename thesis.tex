...
\chapter{Overview of grid point reduction strategies}
...
\todo{Really the same spatially adap. ?}
...
\begin{description}
\item[Coarse boundaries] A compromise between having no boundary and having a full boundary are coarse boundaries.
...
\begin{equation}
f_1(x)=2 x_1 + \frac{1}{10}\sin(2 \pi x_2) + 1
\nonumber
\end{equation}

It is a three-dimensional function, where the first two input components are the ... and should be preserved .
The third input dimension is used, but does not contribute a lot to the overall function and can be kept or removed by the methods, depending on the method configuration.
The fourth and last input dimension is completely unused and should be detected and removed by dimension reduction methods.
...
If there are $n$ functions with $r_{min}(f_i) = r_i, ~ i \in \{1,\dots,n\}$ and the $A_i \in R^{d \times r_i}$ are independent with \todo{Summe bis $n$, nicht $i$ oder?} $f=\sum_{k=1}^i \hat{f}_k \circ t_{A_k}$, then we define a function $f$ as $r$-dimensional representable in $n$ steps, i.e. an optimal algorithm that correctly calculates $A_i$ needs a minimum of $n$ iterations to exactly approximate the function $f$.
